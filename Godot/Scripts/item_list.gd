...
func _input(_event):
	...
	if Input.is_action_just_pressed(&"ui_accept"):
		if (Globals.RPG.combat_state == Globals.RPG.combat_turn.PLAYER_MENU):
			Items[fucking_test_var].quantity -= 1
			Globals.eat_inventory_item.emit(Items[fucking_test_var].item.texture,Items[fucking_test_var])
