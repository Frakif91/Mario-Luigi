class_name Mario_Multiplayer_Movement extends CharacterBody3D
...
# @onready var luigi : Luigi_Multiplayer_Movement = get_node(luigi_np)
...
func _process(_delta):
	if is_multiplayer_authority() and can_play_animation:
...
func get_action_and_direction(cur_direction : Vector2):
	var direction_angle = roundf(rad_to_deg(cur_direction.angle()))
	...
	state_direction = SORTED_DIRECTION[floor(direction_angle/each_index) + 2 % SORTED_DIRECTION.size() ]
