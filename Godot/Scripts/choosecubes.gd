...
@export var distance_margin = 5.0
var is_in_choosing_position = false
var is_moving = false
var choosen_floating_one_delta : float = 0.0
const BLOCKS_NAMES = ["HAMMER","BROS.","FLEE","ITEM","JUMP"]
...
func emit_hit_signal():
	if is_in_choosing_position:
		hit_block.emit()

func _ready():
	$Area3D.body_entered.connect(emit_signal)
	blocks_nb = 0
